...
public class Run {
	...
	public static String getObject(String key, String JSON) {
		...
		//"\"(type)\":\"((\\\"|[^\"])\")"
		Pattern pattern = Pattern.compile(String.format("(\"type\":\"[A-z0-9]*\")", key));
	...
	public static void main(String[] args) throws IOException {
		...
		//br.readLine();
		Pattern escapeChars = Pattern.compile("\"type\":\\s*(.*),");//Pattern.compile("^(\\*|[\\w])*[\\^][\\w]*[\\*]*$");
		Pattern replaceChars = Pattern.compile("\\^");
		Pattern stripChars = Pattern.compile("[:]");

		
		
		String frameText = "{\"type\":\"logout_req\",\"value\":\"help\",\"type\":\"hello\"}";//""*^comp*";
		System.out.println(frameText);
		int quoteIndex = frameText.indexOf(":")+2;
//		System.out.println(frameText.substring(quoteIndex,frameText.indexOf("\"",quoteIndex)));
		System.out.println(getObject("type", frameText));
		StringBuffer sb = new StringBuffer();

		if (escapeChars.matcher(frameText).matches()){
			//			replaceChars.
			sb.append(replaceChars.matcher(stripChars.matcher(frameText).replaceAll(" ").trim()).replaceFirst("\\\\"));
//			System.out.println("Ok"+sb);
		}
//		else
//			System.out.println("NOK"+stripChars.matcher(frameText).replaceAll(" ").trim());
